...
export default function CompletarPerfilPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white relative overflow-hidden py-12">
      ...
      <div className="w-full max-w-2xl mx-4 animate-fade-in z-10">
        ...
        <div className="glass rounded-3xl p-8 shadow-xl shadow-rosa-400/5 border border-gray-100">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="w-32 h-32 rounded-full border-4 border-gray-100 overflow-hidden bg-gray-50 flex items-center justify-center relative group">
                ...
                <div className="absolute inset-0 bg-black/50 hidden group-hover:flex items-center justify-center transition-all">
                  <CldUploadWidget uploadPreset="busco_profe" onSuccess={handleUploadFoto} options={{ maxFiles: 1 }}>
                    {({ open }) => (
              ...
              {!formData.foto && (
                <CldUploadWidget uploadPreset="busco_profe" onSuccess={handleUploadFoto} options={{ maxFiles: 1 }}>
                  {({ open }) => (
                    <button type="button" onClick={() => open()} className="text-sm font-semibold text-rosa-500 hover:text-rosa-600">Subir foto de perfil</button>
                  )}
                </CldUploadWidget>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Teléfono / WhatsApp */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">WhatsApp (con código de país)</label>
                <input
                  name="telefono"
                  type="text"
                  required
            ...
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 space-y-4">
              <h3 className="font-bold text-black flex items-center gap-2">
                <span>📎</span> Material multimedia <span className="text-xs font-normal text-gray-400 bg-gray-200 px-2 py-0.5 rounded-md">(Opcional)</span>
              </h3>
              
              <div className="flex flex-col gap-3">
                <CldUploadWidget uploadPreset="busco_profe" onSuccess={handleUploadVideo} options={{ sources: ['local'], resourceType: 'video', maxFiles: 1 }}>
                  {({ open }) => (
                    <button type="button" onClick={() => open()} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-rosa-300 transition-colors group">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-rosa-50 rounded-lg flex items-center justify-center text-rosa-500 group-hover:scale-110 transition-transform">🎥</div>
                        <div className="text-left">
                          <p className="font-semibold text-sm text-black">Video de presentación</p>
                          <p className="text-xs text-gray-500">{formData.video_url ? "Video subido ✓" : "Sube un video corto presentándote"}</p>
                        </div>
                      </div>
                ...
                <CldUploadWidget uploadPreset="busco_profe" onSuccess={handleUploadTitulo} options={{ sources: ['local'], multiple: true }}>
                  {({ open }) => (
                    <button type="button" onClick={() => open()} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-rosa-300 transition-colors group">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-rosa-50 rounded-lg flex items-center justify-center text-rosa-500 group-hover:scale-110 transition-transform">🎓</div>
                        <div className="text-left">
                          ...
                          <p className="text-xs text-gray-500">{formData.titulos.length > 0 ? `${formData.titulos.length} archivos subidos` : "Demuestra tus conocimientos"}</p>
                        </div>
                      </div>
                      <span className="text-rosa-400 font-bold text-xl">+</span>
                    </button>
            ...
            <button
              type="submit"
              disabled={isLoading || !formData.modalidad}
            ...
            >
              {isLoading ? "Guardando..." : "🚀 Publicar mi perfil"}
            </button>
          </form>
        </div>
      </div>
